...
if(isset($_GET['idFaktura'])){
	...
	$pdf->Cell(55,10, "Nazwa",1,0,'C');
	$pdf->Cell(15,10, "Ilość",1,0,'C');
	...
	while ($produkt = $wynik->fetch_assoc()) {
		...
		$nameWidth = $pdf->GetStringWidth($produkt['nazwa']);
		if($nameWidth>53){
			$dzielnik=round($nameWidth/55) + 1;
			$cellHeight*=$dzielnik;
		}
		
		$pdf->Cell(10,$cellHeight,$i,1,0,'C');
		$pdf->MultiCell(55,$cellHeight/$dzielnik,$produkt['nazwa'],1,'C',0);
		$pdf->SetY($pdf->GetY()-$cellHeight);
		$pdf->SetX($pdf->GetX()+65);
		$pdf->MultiCell(15,$cellHeight,$produkt['ilosc'],1,'C',0);
		$pdf->SetY($pdf->GetY()-$cellHeight);
		$pdf->SetX($pdf->GetX()+80);
		$pdf->MultiCell(17,$cellHeight,$produkt['cena'],1,'C',0);
		$pdf->SetY($pdf->GetY()-$cellHeight);
		$pdf->SetX($pdf->GetX()+97);
		$pdf->MultiCell(25,$cellHeight,$produkt['netto'],1,'C',0);
		$pdf->SetY($pdf->GetY()-$cellHeight);
		$pdf->SetX($pdf->GetX()+122);
		$pdf->MultiCell(13,$cellHeight,$produkt['vat_proc'],1,'C',0);
		$pdf->SetY($pdf->GetY()-$cellHeight);
		$pdf->SetX($pdf->GetX()+135);
		$pdf->MultiCell(21,$cellHeight,$produkt['vat'],1,'C',0);
		$pdf->SetY($pdf->GetY()-$cellHeight);
		$pdf->SetX($pdf->GetX()+156);
		$pdf->MultiCell(25,$cellHeight,number_format($brutto,2,"."," "),1,'C');
		$i++;
		/*
